...
public class GridFsIndexUtil {
  ...
  /**
   * Ensures all required indexes exist on the given GridFS files collection. This method is
  ...
  public static void ensureGridFsIndexes(MongoTemplate mongoTemplate, String collectionName) {
    if (!mongoTemplate.collectionExists(collectionName)) {
      logger.info(
          "Collection '{}' does not exist yet. Skipping index creation. It will likely be created by GridFS operations.",
          collectionName);
      return;
    }
    ...
    IndexDefinition systemFilenameIndex = new Index().on("filename", Sort.Direction.ASC).unique();
    indexOps.ensureIndex(systemFilenameIndex);
    logger.info("Ensured index on 'filename' for collection: {}", collectionName);

    IndexDefinition originalFilenameIndex =
        new Index()
    ...
    logger.info(
        "Ensured index on 'metadata.ownerId' and 'metadata.originalFilename' for collection: {}",
        collectionName);

    IndexDefinition contentHashIndex =
        new Index()
            .on("metadata.ownerId", Sort.Direction.ASC)
    ...
    logger.info(
        "Ensured sparse unique index on 'metadata.ownerId' and 'metadata.sha256' for collection: {}",
        collectionName);

    IndexDefinition tokenIndex = new Index().on("metadata.token", Sort.Direction.ASC).unique();
