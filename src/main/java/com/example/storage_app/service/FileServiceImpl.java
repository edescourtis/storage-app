...
public class FileServiceImpl implements FileService {
    ...
    Query filenameConflictQuery =
        new Query(
            Criteria.where("metadata.originalFilename")
                .is(userProvidedFilename)
                .and("metadata.ownerId")
                .is(userId));
    if (mongoTemplate.exists(filenameConflictQuery, "fs.files")) {
      throw new FileAlreadyExistsException(
          "Filename '" + userProvidedFilename + "' already exists for this user.");
    }
    ...
    try (InputStream actualStream = detectedMime.stream;
      ...
      try {
        ...
        if (storedFileObjectId == null) {
          throw new StorageException(
              "Failed to store file, GridFS store operation returned null ID.");
        }
      } catch (Exception e) {
        if (e instanceof DuplicateKeyException
            || (e instanceof MongoWriteException
                && ((MongoWriteException) e).getError().getCode() == 11000)) {
          String errorMessage = e.getMessage() != null ? e.getMessage().toLowerCase() : "";
          logger.debug(
              "MongoDB DuplicateKeyException or E11000 WriteException during GridFS store for systemFilenameUUID: {}. Message: {}",
              systemFilenameUUID,
              e.getMessage(),
              e);
          if (errorMessage.contains("metadata.originalfilename")) {
            throw new FileAlreadyExistsException(
                "Filename '"
                    ...
                    + "' already exists for this user (DB conflict during store).",
                e);
          } else if (errorMessage.contains("metadata.sha256")) {
            throw new FileAlreadyExistsException(
                "Content already exists for this user (DB conflict during store).", e);
          }
          throw new StorageException(
              "Failed to store file due to a data conflict during initial storage (GridFS store).",
              e);
        ...
        logger.error(
            "Unexpected exception during GridFS store for systemFilenameUUID: {}. Exception: {}",
            systemFilenameUUID,
            e.getMessage(),
            e);
        throw new StorageException(
            "Failed to store file due to an unexpected issue during initial storage.", e);
      }
      ...
      try {
        UpdateResult updateResult =
            mongoTemplate.updateFirst(queryForHashUpdate, updateWithHash, "fs.files");
        if (updateResult.getModifiedCount() == 0) {
          logger.warn(
              "File metadata update for hash failed for ID: {}. Zero documents modified. Potential orphan if transaction doesn't roll back GridFS store.",
              storedFileObjectId);
          throw new StorageException(
              "File metadata update for hash failed for ID: "
                  + storedFileObjectId
                  + ". Zero documents modified.");
        }
      } catch (org.springframework.dao.DuplicateKeyException e) {
        logger.warn(
            "Content duplicate detected for user {} (hash: {}) upon metadata update. SystemFileUUID: {}. Attempting to delete orphaned GridFS file.",
            userId,
            hash,
            systemFilenameUUID);
        try {
          ...
          logger.info(
              "Successfully deleted orphaned GridFS file (id: {}) after hash conflict.",
              storedFileObjectId);
        } catch (Exception deleteEx) {
          logger.error(
              "Failed to delete orphaned GridFS file (id: {}) after hash conflict. Manual cleanup may be required.",
              storedFileObjectId,
              deleteEx);
        }
        throw new FileAlreadyExistsException(
            "Content already exists for this user (hash conflict: " + e.getMessage() + ")", e);
      }
  ...
  public Page<FileResponse> listFiles(
    ...
    List<FileResponse> fileResponses =
        fileDocuments.stream().map(this::mapDocToFileResponse).collect(Collectors.toList());
  ...
  public FileResponse updateFileDetails(String userId, String fileId, FileUpdateRequest request) {
    ...
    if (!StringUtils.hasText(newFilenameFromRequest)
        || newFilenameFromRequest.equals(currentOriginalFilename)) {
    ...
    if (mongoTemplate.exists(conflictQuery, "fs.files")) {
      throw new FileAlreadyExistsException(
          "Filename '" + newFilenameFromRequest + "' already exists for this user.");
    }
    ...
    com.mongodb.client.result.UpdateResult updateResult =
        mongoTemplate.updateFirst(findBySystemUUIDQuery, updateDefinition, "fs.files");
  ...
  private FileResponse mapDocToFileResponse(Document fsFileDoc) {
    ...
    try {
      visibilityEnum =
          StringUtils.hasText(visString)
              ? Visibility.valueOf(visString.toUpperCase())
              : Visibility.PRIVATE;
    } catch (IllegalArgumentException e) {
      logger.warn(
          "Invalid visibility string '{}' in metadata for system ID {}. Defaulting to PRIVATE.",
          visString,
          systemFileId);
